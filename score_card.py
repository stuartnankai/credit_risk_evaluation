...
# Discretization
def discretization_auto(Y, X, n=20):
    global d2
    r = 0
    positive = Y.sum()
    negative = Y.count() - positive
    while np.abs(r) < 1:
    ...
    d3['goodattribute'] = d3['sum'] / positive
    d3['badattribute'] = (d3['total'] - d3['sum']) / negative
    ...
    print("This is woe: ", woe)
    return d4, iv, cut, woe


# user-defined
def discretization_self(Y, X, cat):
    good = Y.sum()
    bad = Y.count() - good
...
if __name__ == '__main__':
    ...
    pinf = float('inf')  # positive big number
    ninf = float('-inf')  # negative big number
    dfx1, ivx1, cutx1, woex1 = discretization_auto(data.SeriousDlqin2yrs, data.RevolvingUtilizationOfUnsecuredLines,
                                                   n=10)
    dfx2, ivx2, cutx2, woex2 = discretization_auto(data.SeriousDlqin2yrs, data.age, n=10)
    dfx4, ivx4, cutx4, woex4 = discretization_auto(data.SeriousDlqin2yrs, data.DebtRatio, n=20)
    dfx5, ivx5, cutx5, woex5 = discretization_auto(data.SeriousDlqin2yrs, data.MonthlyIncome, n=10)
    # discretization
    ...
    dfx3, ivx3, woex3 = discretization_self(data.SeriousDlqin2yrs, data['NumberOfTime30-59DaysPastDueNotWorse'], cutx3)
    dfx6, ivx6, woex6 = discretization_self(data.SeriousDlqin2yrs, data['NumberOfOpenCreditLinesAndLoans'], cutx6)
    dfx7, ivx7, woex7 = discretization_self(data.SeriousDlqin2yrs, data['NumberOfTimes90DaysLate'], cutx7)
    dfx8, ivx8, woex8 = discretization_self(data.SeriousDlqin2yrs, data['NumberRealEstateLoansOrLines'], cutx8)
    dfx9, ivx9, woex9 = discretization_self(data.SeriousDlqin2yrs, data['NumberOfTime60-89DaysPastDueNotWorse'], cutx9)
    dfx10, ivx10, woex10 = discretization_self(data.SeriousDlqin2yrs, data['NumberOfDependents'], cutx10)
